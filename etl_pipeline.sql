...
CREATE TABLE IF NOT EXISTS fact_order (
  ...
  order_nk        TEXT UNIQUE,        -- deterministic natural key
...
/* ===================================================================================================================================
   2) ETL PIPELINE - DATA TRANSFORMATION & LOADING
   ==================================================================================================================================== */
...
SET name               = (SELECT u.name            FROM stg_users u WHERE CAST(u.user_id AS TEXT) = dim_customer.user_id_nat),
    ...
    education_raw      = (SELECT u."Educational qualifications" FROM stg_users u WHERE CAST(u.user_id AS TEXT) = dim_customer.user_id_nat)
WHERE EXISTS (SELECT 1 FROM stg_users u WHERE CAST(u.user_id AS TEXT) = dim_customer.user_id_nat);

-- Transform income groups (idempotent)
UPDATE dim_customer
SET
  income_group = CASE
...
-- Transform education groups (idempotent)
UPDATE dim_customer
...
/* =======================================================================
   6) DIM LOCATION — INSERT-OR-IGNORE then UPDATE
   ======================================================================= */
...
/* =======================================================================
   7) FACT LOAD (update existing rows, then insert new ones)
...
-- 7A) UPDATE existing fact rows (idempotent refresh)
UPDATE fact_order
...
-- 7B) INSERT new fact rows (ignore duplicates)
...
-- 7C) Update location keys in fact table
...
-- Transform Currency from USD to INR (exchange rate: 1 USD = 87 INR)
UPDATE fact_order
SET sales_amount = sales_amount * 87,
    currency = 'INR'
WHERE LOWER(currency) = 'usd';

-- Clean invalid sales amounts
DELETE FROM fact_order
WHERE sales_amount <= 0;

/* =======================================================================
   9) INDEXES (safe to re-run)
   ======================================================================= */
...
-- Table counts
SELECT 'SUMMARY: Table Counts' AS check_type;
...
-- FK orphan checks (should all return 0)
SELECT 'DATA QUALITY: Foreign Key Orphans' AS check_type;
